...
namespace WindowsFormsApp1.Forms
{
    public partial class InternetAndTVPaymentsForm : Form
    {
        ...
        private void InternetAndTVPaymentsForm_Load(object sender, EventArgs e)
        {
            ...
            InternetbunifuDropdown.DataSource = operators;
            //InternetComboBox.ValueMember = "id_service";
            InternetbunifuDropdown.ValueMember = "id_service";
            //InternetComboBox.DisplayMember = "serviceName";
            InternetbunifuDropdown.DisplayMember = "serviceName";
        ...
        private void PayButton_Click(object sender, EventArgs e)
        {
            ...
            var PersonalAccount = PersonalAccountbunifuTextBox.Text;
            ...
            if (error == false)
            {
                ...
                if (DataStorage.attempts > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into transactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values('Оплата услуг Интернет и ТВ', '{InternetbunifuDropdown.GetItemText(InternetbunifuDropdown.SelectedItem)}', '{transactionDate}', '{transactionNumber}', '{sum}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";
                    ...
                    var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum}' where serviceName = '{InternetbunifuDropdown.GetItemText(InternetbunifuDropdown.SelectedItem)}' and serviceType = 'Internet'";
                    ...
                    var queryTransaction4 = $"insert into clientPersonalAccount(personal_account, id_service, id_client) values('{PersonalAccountbunifuTextBox.Text}', (select id_service from clientServices where serviceName = '{InternetbunifuDropdown.GetItemText(InternetbunifuDropdown.SelectedItem)}'), '{DataStorage.idClient}')";
