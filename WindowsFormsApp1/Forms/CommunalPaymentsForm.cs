...
namespace WindowsFormsApp1.Forms
{
    public partial class CommunalPaymentsForm : Form
    {
        ...
        private void CommunalPaymentsForm_Load(object sender, EventArgs e)
        {
            ...
            CommunalbunifuDropdown.DataSource = operators;
            //CommunalComboBox.ValueMember = "id_service";
            CommunalbunifuDropdown.ValueMember = "id_service";
            //CommunalComboBox.DisplayMember = "serviceName";
            CommunalbunifuDropdown.DisplayMember = "serviceName";
        ...
        private void PayButton_Click(object sender, EventArgs e)
        {
            ...
            if (error == false)
            {
                ...
                if (DataStorage.attempts > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into transactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values('Оплата коммунальных услуг', '{CommunalbunifuDropdown.GetItemText(CommunalbunifuDropdown.SelectedItem)}', '{transactionDate}', '{transactionNumber}', '{sum}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";
                    ...
                    var queryTransaction3 = $"update clientServices set serviceBalance = serviceBalance + '{sum}' where serviceName = '{CommunalbunifuDropdown.GetItemText(CommunalbunifuDropdown.SelectedItem)}' and serviceType = 'Communal'";
                    ...
                    var queryTransaction4 = $"insert into clientPersonalAccount(personal_account, id_service, id_client) values('{PersonalAccountbunifuTextBox.Text}', (select id_service from clientServices where serviceName = '{CommunalbunifuDropdown.GetItemText(CommunalbunifuDropdown.SelectedItem)}'), '{DataStorage.idClient}')";
